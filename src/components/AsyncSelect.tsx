...
export default function AsyncSelect({
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setIsLoading(true);
            try {
                const response = await fetch(`${API_BASE_URL}${apiEndpoint}?search=${encodeURIComponent(debouncedSearch)}&page=1`);
                ...
                console.error("Gagal load data Select:", err);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    return (
        <div className="relative w-full space-y-2" ref={dropDownRef}>
            {label && <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-4">{label}</label>}
            ...
            >
                <span className={`text-sm font-bold ${!selectedLabel ? 'text-gray-400' : 'text-gray-700'}`}>
            ...
            {error && <p className="text-[10px] text-red-500 font-bold ml-4 uppercase">{error}</p>}
        </div>
    );
